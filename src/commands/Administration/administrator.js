...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      ...
      aliases: ['setadmin', 'manageadmin', 'admin'],
  ...
  async add(message, admin) {
    ...
    if (type === 'member') {
      if (message.guild.settings.get('users.admin').includes(memberOrRole.id)) throw message.language.get('CMD_ADMINS_EXISTS', memberOrRole);
      await message.guild.settings.update('users.admin', memberOrRole, { arrayAction: 'add', guild: message.guild });
      // return message.channel.send(message.language.get('CMD_ADMINS_ADDED', memberOrRole));
    }

    if (type === 'role') {
      if (message.guild.settings.get('roles.admin') === memberOrRole.id) throw message.language.get('CMD_ADMINS_EXISTS', memberOrRole);
      await message.guild.settings.update({ roles: { admin: memberOrRole } }, message.guild);
      // return message.channel.send(message.language.get('CMD_ADMINS_ADDED', memberOrRole.name));
    }
