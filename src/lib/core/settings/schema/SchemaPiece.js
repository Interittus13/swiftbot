const { isFn, isNum } = require('../../util/util');
...
class SchemaPiece {
  ...
  isValid() {
    ...
    // Check limits
    if (this.min !== null && !isNum(this.min)) throw new TypeError(`[KEY] ${this.path} - Parameter min must be a number or null.`);
    if (this.max !== null && !isNum(this.max)) throw new TypeError(`[KEY] ${this.path} - Parameter max must be a number or null.`);
    ...
    // Check filter
    if (this.filter !== null && !isFn(this.filter)) throw new TypeError(`[KEY] ${this.path} - Parameter filter must be a function`);
